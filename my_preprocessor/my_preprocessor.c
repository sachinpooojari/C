...
//	printf(" ?<%s>? ",s);;
	...
	char cmp_str[MAX];Z
	while(fscanf(fp,"%s",cmp_str)!=-1)
	{
		char* tmp;  // to hold  address of modifiable text
		

		if(tmp=strstr(cmp_str,str))
		{
			//fseek(fp,strlen(cmp_str),SEEK_CUR);  // to move back poiter
			//strcpy(tmp,data);
			int i;
			for(i=0 ;tmp[i];i++ )
			{				
...
void remove_comment(FILE *fp,FILE *dp)
{
	...
	{	//embedd here error code 
		...
		{ 
			ch1=fgetc(fp);
			if(ch1=='/')
				fgets(tmp,MAX,fp); /// to send poinerr to end of line 
			else if(ch1=='*')
			{
...
{	
	...
	fclose(dp);
	fclose(fp);
// at this stage all comments are removd 
	...
	while( fscanf(fp,"%s",s) != EOF)
	{
		if(strstr(s,"#include"))
		{
			...
			F_pointer_fp=ftell(fp);
			F_pointer_dp=ftell(dp);
		}
//		printf("%s ",s);
	}

	fseek(fp,F_pointer_fp,SEEK_SET);
			//to check from begining of include inside 


	fseek(fp,F_pointer_fp,SEEK_SET);
	printf("%d",F_pointer_fp);
	char ch;
	while((ch=getc(fp))!=EOF)
		fputc(ch,dp); 
	fclose(fp);
	fclose(dp);
	// at this stage all include are added to .i file 

	fp=fopen(tmp,"r");   // sp -> .i file become source 
	dp=fopen("final","w");
